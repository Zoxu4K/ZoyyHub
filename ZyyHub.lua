...
-- Load ZyyLoader
local ZyyLoader = loadstring(game:HttpGet("https://github.com/Zoxu4K/ZoyyHub/blob/main/Loader.lua"))()

if not ZyyLoader then
    LoadingNotification.Complete(false, 0, 1)
    SendNotification("❌ ERROR", "ZyyLoader failed!", 10)
    return
end

LoadingNotification.Update(1, 32, "ZyyLoader")
...
local function LoadModuleWithRetry(moduleName, retryCount)
    ...
    local success, result = pcall(function()
        return ZyyLoader.LoadModule(moduleName)
    end)
